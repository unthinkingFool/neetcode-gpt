...
class Solution:
    def batch_loader(self, raw_dataset: str, context_length: int, batch_size: int) -> Tuple[List[List[str]], List[List[str]]]:
        ...
        indices=torch.randint(0,len(tokens)-context_length,(batch_size,))

        X=[
            tokens[start.item():start.item()+context_length]
            for start in indices
        ]
        y=[
            tokens[start.item()+1:start.item()+1+context_length]
            for start in indices
        ]
